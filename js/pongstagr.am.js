  ...
  Pongstgrm.prototype.template = {
    ...
    , profile: function (options) {
        var _profile  = '<div class="row pongstgrm-profile">'       
            _profile += ' <div class="col-xs-12 text-left">'
            _profile += '     <div class="thumbnail"><img class="round" src="'+ options.profile_picture +'" width="'+ options.picture_size +'"  height="'+ options.picture_size +'" alt="'+ options.username +'"></div>'
            _profile += '     <div class="title">'+ options.username +'</div>'
            _profile += '     <span class="counts" align="left">'
            ...
            _profile += '     <p>'+ options.full_name +' - <a href="'+ options.website +'">' + options.website +'</a></p>'
            _profile += '     <small>'+ options.bio +'</small>'
            _profile += '     </div>'
            _profile += '   </div>'
            _profile += '   </div>'
            _profile += ' </div>'
    ...
    , thumb: function (options) {
        var _thumbnail  = '<div class="'+ options.dflt.column +'">'
          ...
          options.dflt.timestamp !== false ?
            ...
            _thumbnail += '   <a class="noline" href="#'+ options.data.id +'" id="triggr-'+ options.data.id +'">'
    ...
    , bsmodal: function (options) {
        ...
        var video  = '<video id="'+ options.data.id +'-video" width="100%" height="auto">'
            ...
            video +=  alert
            video += '</video>'
            video += '<button type="button" class="button" id="play-pause"><i class="glyphicon glyphicon-play"></i></button>'
        ...
        var modal  = '<div id="'+ options.data.id +'" class="modal fade">'
          ...
          navigator.userAgent.match(/webkit/i) || navigator.userAgent.match(/(iPod|iPhone|iPad)/) ?
            ...
            modal += '  <a class="noline" href="https://instagram.com/'+ options.data.username +'" class="media-object thumbnail pull-left">'
            ...
            modal += '        <a class="noline" href="https://instagram.com/'+ options.data.username +'">'+ options.data.username +'</a>'
            modal += '      </h5>'
          ...
          options.data.comments_count !== 0 ?
            $.each(options.data.comments_data, function(a, b){
              modal += '<div class="media">'
              modal += '  <a class="noline" href="https://instagram.com/'+ b.from.username +'" class="media-object thumbnail pull-left">'
              modal += '    <img src="'+ b.from.profile_picture +'" width="'+ options.dflt.picture_size +'" height="'+ options.dflt.picture_size +'">'
              modal += '  </a>'
              modal += '  <div class="modal-body">'
              modal += '      <h5 class="media-heading">'
              modal += '        <a class="noline" href="https://instagram.com/'+ b.from.username +'">'+ b.from.username +'</a>'
              modal += '      </h5>'
              modal += '      <p>'+ b.text +'</p>'
              modal += '  </div>'
              modal += '</div>'
